...
void Setup_MPins(){
  ...
  digitalWrite(REN0,HIGH);//enable left
  digitalWrite(LEN0,HIGH);//enable right 
  //pinMode (PWM, OUTPUT);
}

int SMap(int val){
  val = map(val,CMin,CMax,MMin,MMax);
}

void Tank_Drive(int FB,int LR){// attach Control Chan for Forw/Bac and Lft/Rght
 int CFB = SMap(FB); //clean for/bac and set 1500 to 0 for center stick 
 int CLR = SMap(LR); //clean lft/rght and set 1500 to 0 for center stick
